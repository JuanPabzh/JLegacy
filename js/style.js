...
async function cargarProductos() {
  const res = await fetch(`${SUPABASE_URL}/rest/v1/productos?tienda=eq.style&select=*`, {
    headers: { 'apikey': SUPABASE_KEY, 'Authorization': `Bearer ${SUPABASE_KEY}` }
  });
...
function buildWALink(product, selectedColor) {
  ...
  const msg = `Hola! Me interesa *${product.nombre}* (${product.categoria}) en color ${colorName}. Precio: $${formatPrice(product.precio)} COP. ¿Está disponible?`;
...
function renderProducts(products) {
  ...
  filtered.forEach((product, i) => {
    ...
    const imgSrc = product.imagen_url || 'assets/style-placeholder.jpg';
 
    card.innerHTML = `
      ${badge}
      <div class="card-img-wrap">
        <img src="${imgSrc}" alt="${product.nombre}" onerror="this.src='assets/style-placeholder.jpg'">
